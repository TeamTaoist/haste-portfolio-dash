...
class AssetInfoManager {
  ...
  private async findXudtInfo(xudtTS: Script) {
    ...
    const idx = BI.from(txInfo.objects[0].txIndex).toNumber();

    if (ckTxInfo && ckTxInfo.data.attributes.display_outputs_count >= 3) {
      ...
      ckTxInfo &&
      ckTxInfo.data.attributes.display_outputs[idx].extra_info
    ) {
  ...
  getXUDTInfo(xudtTS: Script) {
    ...
    if (!info) {
      ...
      this.findXudtInfo(xudtTS).catch((err) => {
        console.error(err);
      });
    } else {
