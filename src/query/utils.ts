...
export const calculateUdtCellCapacity = (lock: Script, udtType: Script)  => {
...
// minimum occupied capacity and 1 ckb for transaction fee
export const calculateNFTCellCapacity = (lock: Script, cell: Cell) => {
  const lockArgsSize = remove0x(lock.args).length / 2;
...
// minimum occupied capacity and 1 ckb for transaction fee
export const calculateEmptyCellMinCapacity = (lock: Script) => {
  const lockArgsSize = remove0x(lock.args).length / 2;
